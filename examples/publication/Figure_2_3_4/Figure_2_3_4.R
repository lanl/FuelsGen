...
load(here::here('examples/publication/Figure_2_3_4/mcmc_n_10_y_cov_avg_mets.RData'))
...
for(i in 1:4){
  ...
  quant.obs = cbind(quantile(theta.n.1.avg.mets[,i],c(.025,.975)),
                    quantile(theta.n.10.avg.mets[,i],c(.025,.975)),
  ...
  if(i==2)
    legend("topright", inset = c(0, 0),                   # Create legend outside of plot
           legend = c("Data","Data+Gam(3,1)","Data+Unif(0,10)"),
           lty=1,
           col = c('darkorange','cornflowerblue','forestgreen'))
}
dev.off()
